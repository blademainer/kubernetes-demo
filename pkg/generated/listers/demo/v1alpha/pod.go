...
type PodLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha.Pod, err error)
...
// List lists all Pods in the indexer.
func (s *podLister) List(selector labels.Selector) (ret []*v1alpha.Pod, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha.Pod))
	})
	return ret, err
}
...
type PodNamespaceLister interface {
	...
	Get(name string) (*v1alpha.Pod, error)
...
// List lists all Pods in the indexer for a given namespace.
func (s podNamespaceLister) List(selector labels.Selector) (ret []*v1alpha.Pod, err error) {
...
// Get retrieves the Pod from the indexer for a given namespace and name.
func (s podNamespaceLister) Get(name string) (*v1alpha.Pod, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha.Resource("pod"), name)
	}
	return obj.(*v1alpha.Pod), nil
}
